...
class TaxPersistenceSpockTest extends SpockPersistenceTestAbstractClass {
	...
	def thenAssert() {
		...
		for(Invoice invoice : irs.getTaxPayerByNIF(SELLER_NIF).getInvoice_sellerSet()) {
			if (invoice.getSeller().getNif().equals(taxPayer1.getNif())) {
				assert SELLER_NIF == taxPayer1.getNif()
			}
		}
		for(Invoice invoice : irs.getTaxPayerByNIF(BUYER_NIF).getInvoice_buyerSet()) {
			if (invoice.getBuyer().getNif().equals(taxPayer1.getNif())) {
		...
		for(Invoice invoice : irs.getTaxPayerByNIF(SELLER_NIF).getInvoice_sellerSet()) {
			if (invoice.getSeller().getNif().equals(taxPayer2.getNif())) {
				assert SELLER_NIF == taxPayer2.getNif()
			}
		}

		for(Invoice invoice : irs.getTaxPayerByNIF(BUYER_NIF).getInvoice_buyerSet()) {
			if (invoice.getBuyer().getNif().equals(taxPayer2.getNif())) {
