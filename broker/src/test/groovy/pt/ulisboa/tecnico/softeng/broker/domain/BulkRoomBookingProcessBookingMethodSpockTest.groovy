...
class BulkRoomBookingProcessBookingMethodSpockTest extends SpockRollbackTestAbstractClass {
    ...
    def populate4Test() {
        ...
        bulk = new BulkRoomBooking(broker, NUMBER_OF_BULK, BEGIN, END, NIF, IBAN_BUYER)
    }

    def 'success'() {
        given: 'the hotel interface returns two references for booking'
        hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, NIF, IBAN_BUYER,
    ...
    def 'success twice'() {
        ...
        0 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, NIF, IBAN_BUYER,
                bulk.getId())
        bulk.getReferences().size() == 2
    ...
    def 'one hotel exception'() {
        ...
        1 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, NIF, IBAN_BUYER,
    ...
    def 'max minus one hotel exception'() {
        ...
        (BulkRoomBooking.MAX_HOTEL_EXCEPTIONS - 1) * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, NIF, IBAN_BUYER,
                bulk.getId()) >> { throw new HotelException() }
    ...
    def 'remote exception value is reset by hotel exception'() {
        ...
        (BulkRoomBooking.MAX_REMOTE_ERRORS - 1) * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, NIF, IBAN_BUYER,
                bulk.getId()) >> { throw new RemoteAccessException() }
